defmodule CANLink.BLE do
  ...
  defmodule LocationService do
    ...
    def subscribe(:location) do
      ...
      Phoenix.PubSub.subscribe(CANLink.PubSub, "location")
    end

    def unsubscribe(:location) do
      Logger.info("unsubscribe")
      Phoenix.PubSub.unsubscribe(CANLink.PubSub, "location")
    end

    def notify(%{latitude: lat, longitude: lon}) do
      Logger.info("notify")
      <<lat::little-float-64, lon::little-float-64>>
    end
  end
  ...
  def init(args) do
    ...
    data = %{port: port, enable: enable, context: nil, peripheral: nil, subscriptions: []}
  ...
  def advertise(:cast, :disable, data) do
    data = do_disable(data)
    {:next_state, :reset, data, []}
  end

  def advertise(:info, {Peripheral, :connected}, data) do
    Logger.info("New BLE connection")
    _ = File.write("/sys/class/leds/beaglebone:green:usr0/trigger", "pattern")
    _ = File.write("/sys/class/leds/beaglebone:green:usr0/pattern", "0 1000 0 0 255 2000 255 0")
  ...
  def connected({:call, from}, {:subscribe, mod, id}, data) do
    _ = mod.subscribe(id)
    {:keep_state, %{data | subscriptions: [{mod, id} | data.subscriptions]}, [{:reply, from, :ok}]}
  end
  ...
  def connected({:call, from}, {:unsubscribe, mod, id}, data) do
    ...
    subscriptions =
      Enum.reject(data.subscriptions, fn
        {^mod, ^id} -> true
        _ -> false
      end)

    {:keep_state, %{data | subscriptions: subscriptions}, [{:reply, from, :ok}]}
  end

...
esp_err_t pico_sync(pico_t* pico)
{
    ...
    pico_send_command(pico, &command);
    return err;
}
...
command_response_t pico_send_command(pico_t* pico, pico_command_t* command)
{
    ...
    for(uint8_t i = 0; i < 4; i++) {
        ESP_LOGD(TAG, "sending %d %02X", i, t[i].tx_data[0]);
        err = spi_device_polling_transmit(pico->spi, &t[i]);
        ESP_LOGD(TAG, "got %d %02X", i, command->rx_buffer[i]);
        ESP_ERROR_CHECK(err);
        if(command->rx_buffer[i] != 0xfe) {
            ESP_LOGD(TAG, "probably out of sync %02X", command->rx_buffer[i]);
    ...
    err = spi_device_polling_transmit(pico->spi, &t[4]);
    ESP_ERROR_CHECK(err);
    ESP_LOGD(TAG, "reply={%02X, %02X, %02X, %02X, %02X}", command->rx_buffer[0], command->rx_buffer[1], command->rx_buffer[2], command->rx_buffer[3], command->rx_buffer[4]);

...
#define TEST_DEVICE_NAME            "CONERGB"
...
void example_write_event_env(esp_gatt_if_t gatts_if, prepare_type_env_t *prepare_write_env, esp_ble_gatts_cb_param_t *param) {
    ...
    ESP_LOGI(TAG, "handle=%02X", param->write.handle);
    if(param->write.handle == 0x2a && param->write.len == 3) {
        // uint8_t r = (param->write.value >> 16) && 0xff;
        // uint8_t g = (param->write.value >> 8) && 0xff;
        // uint8_t b = param->write.value && 0xff;
        uint8_t r = param->write.value[0];
        uint8_t g = param->write.value[1];
        uint8_t b = param->write.value[2];
        ESP_LOGI(TAG, "set color %02X, %02X, %02X", r, g, b);
        pico_set_color(pico, COMMAND_RGB_INDEX_0, r, g, b);
    } else if(param->write.handle == 0x2e) {
        ESP_LOGI(TAG, "set brightness");
        pico_set_brightness(pico, COMMAND_RGB_INDEX_0, param->write.value[0]);
    }

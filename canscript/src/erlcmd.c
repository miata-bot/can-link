...
void erlcmd_init(struct erlcmd *handler,
                 void (*request_handler)(size_t len, const char *req, void *cookie),
                 void *cookie)
{
...
void erlcmd_send(char *response, size_t len)
{
    ...
    size_t wrote = 0;
    do {
        ssize_t amount_written = write(4, response + wrote, len - wrote);
        if (amount_written < 0) {
            if (errno == EINTR)
                continue;
...
static size_t erlcmd_try_dispatch(struct erlcmd *handler)
{
    ...
    handler->request_handler(msglen, handler->buffer+sizeof(uint16_t), handler->cookie);
...
{
    ssize_t amount_read = read(3, handler->buffer + handler->index,

defmodule SocketCANTest do
  ...
  setup do
    :os.cmd('rm *.log')
    {:ok, can1} = SocketCAN.start_link()
    Logger.info "can1 is: #{inspect can1}"
    {:ok, can2} = SocketCAN.start_link()
    Logger.info "can2 is: #{inspect can2}"
    {:ok, %{can1: can1, can2: can2}}
  end

  #  test "basic write + read", %{can1: can1, can2: can2} do
  #    :ok = SocketCAN.open(can2, @can2_interface)
  #    :ok = SocketCAN.open(can1, @can1_interface, sndbuf: 1024)
  #    <<id1::size(32)>> = <<1, 2, 3, 4>>
  #    <<id2::size(32)>> = <<4, 3, 2, 1>>
  #    frame1 = {id1, <<97,97,97,97,97,97,97,97>>}
  #    #pad short frames
  #    frame2 = {id2, <<98,98,98,98,98,98,98>>}
  #    :ok = SocketCAN.write(can1, [frame1, frame2])
  #    :ok = SocketCAN.await_read(can2)
  #    receive do
